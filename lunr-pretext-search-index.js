...
},
{
  ...
  "type": "Chapter",
  "number": "1",
  "title": "Introduction To Taxicab Geometry",
  "body": " Introduction To Taxicab Geometry   What Is Taxicab Geometry  A standard reference on taxicab geometry for those new to the subject is . Krause begins his treatise by saying:   The usual way to describe a (plane) geometry is to tell what its points are, what its lines are, how distance is measured, and how angle measure is determined. Note that Krause is talking about a geometry here, not about geometry in general. I should pursue this but instead I am just going to blissfully skip over it.    The standard geometry taught in schools is plane Euclidean geometry. Intuitively, this geometry can be thought of as the geometry of a sheet of paper (the Euclidean plane). The points, then, are the (infinitesimal) dots that make up a plane. The lines are those paths that can be drawn with a straight edge. Distances can be measured using a suitable ruler and angle measures can be determined using a protractor. I will use the symbol to denote the Euclidean plane.  Further, if a rectangular coordinate system is drawn on the sheet of paper then the points on the plane can be uniquely identified by their coordinates with respect to that coordinate system, lines can be identified via a linear equation in two variables and distances can be calculated via Pythagoras' Theorem. Angle measures can also be calculated, rather than measured via a protractor say, but not quite so simply.  I will refer to the study of the geometry of the Euclidean plane via the use of a rectangular coordinate system as Euclidean coordinate geometry or either just Euclidean geometry or co-ordinate geometry . (You will also see it referred to as analytic geometry or Cartesian geometry ). Euclidean co-ordinate geometry, then, is the starting place for our journey into taxicab geometry.  Taxicab geometry, as introduced in , is exactly the same as Euclidean coordinate geometry except in the way that distance is calculated. So, points are still the (infinitesimal) dots that make up the plane and they are still uniquely identified by their coordinates with respect to a set of rectangular axes. Lines are still those paths that can be drawn with a straight edge and angle measures can still be determined using a protractor.  Recall that in co-ordinate geometry the distance between the two points and in the plane, which we will denote by the symbol , is calculated via the formula . I will refer to this distance as the Euclidean distance between the points, (hence the subscript in the symbol).  In taxicab geometry, the distance between the two points, which I will denote by , is now calculated via the formula . I will refer to this distance as the taxicab distance  taxicab distance between the points Both of these formulas are examples of what is known as a metric . Once again I am going to happily skip over this. .  To get an intuitive idea of what this means look at . This figure is a GeoGebra animation and initially just the points and are shown. Now, the Euclidean distance between the two points and would be measured along the straight line path that joins the two points. Thus, . The taxicab distance, however, is . This distance is equivalent to measuring the distance along the shortest path from to that always stays parallel to either the -axis or the -axis. Note that this path is not necessarily unique as you will see if you click the Draw a taxicab path button in several times.   Taxicab paths between two points    So, if we were to imagine that the grid lines in were the streets of some city then we could think of the Euclidean distance between the points and as the distance along the path that a crow would fly between the points and we could think of the taxicab distance as the distance along the path that the (honest) taxicab driver would drive. So I guess that it seems reasonable that this geometry came to be (affectionately) known as taxicab geometry.  Already we can see a difference between Euclidean geometry and taxicab geometry. In Euclidean geometry the path that is used to determine the distance between two points is along the line joining and , and it is unique. In taxicab geometry the path that is used to determine the distance between two points isn't necessarily unique, as you might have seen in . So, let's call any path between two points and that is used to determine the taxicab distance between the points a taxicab path . taxicab path Further, let's call the set of all possible taxicab paths for the two points and the taxicab set  taxicab set for the points and .  Note that in the points and were constrained to have integer co-ordinates and the taxicab path between the points were constrained to only go along the grid lines. This doesn't have to be the case, the points can be anywhere in the plane and taxicab paths are constrained only to always stay parallel to either the -axis or the -axis. I made the additional constraints in the figure simply to make the connection to taxicabs seem a little more plausible!  So in taxicab geometry then, you should be able to convince yourself that the taxicab set for two points and will be a either a rectangle with and at opposite vertices or an interval joining and . Further, when the taxicab set is an interval the taxicab path is unique.  In closing this first discussion of taxicab geometry it is worth pointing out explicitly that when discussing taxicab geometry we will often resort to using the language and the tools of (the familiar) Euclidean co-ordinate geometry. This is only natural, we will use what we know to explore what we don't know. Of course, in places it will be easy to confuse whether we are talking about taxicab geometry results or Euclidean geometry results (or maybe both). Hopefully, care with language, terminology, symbols and other details will minimise such confusion.    Taxicab Circles  To get an insight into the nature of taxicab geometry, this section introduces the idea of circles in taxicab geometry. The plan of attack is to begin by looking at how circles arise in Euclidean geometry and then to use that to guide the development of the idea in taxicab geometry. Note that this will be the plan of attack for all that we use for all of the topics that we will explore in taxicab geometry. However, as you will see, along the way we will come across places where we will have to make decisions on how to proceed with the development of some ideas in taxicab geometry. Because taxicab geometry isn't widely used there are many such places where there is no weight of evidence or thought pointing to what would be the most advantageous way to proceed. In such cases, we are free to make to whatever decision seems sensible at the time. It could be that later on we realise that our decision wasn't a good one for whatever reason. In which case we can go back, change it, (with the benifit of some insight), and see what happens. I have to say doing this turns out to be rather fun (and, dare I say it, creative )!  The Wikipedia article on the Circle defines a circle as follows:   A circle is a shape consisting of all points in a plane that are at a given distance from a given point, the centre. The distance between any point of the circle and the centre is called the radius.   The emphasis in the above quote is mine I wanted to highlight that this definition is in terms of distance. Of course, the distance that the article is referring to is the Euclidean distance. So, let the given point, ( the centre), be and let the given distance, ( the radius),be . (So , and are known quantities.) Then, if is some point on the circle then using Equation we have which can be rearranged to give the equation of a (Euclidean) circle with centre and radius , that is: .  Repeating this process but using taxicab distance formula instead of the Euclidean distance formula gives the equation of a taxicab circle , taxicab circle namely . uses GeoGebra to show a plot of the taxicab circle with centre with centre and radius . As you can see it is somewhat different to the Euclidean circle with the same centre and radius. You should take the time to check that the points on the taxicab circle are indeed a taxicab distance of 2 away from and that there are no other points that are a taxicab distance of 2 away from .   Taxicab circle with centre C and radius r    If you were to plot the graph of the taxicab circle with centre with centre and radius by hand you would have to plot the graph of the equation . The graph of this equation is probably easier seen if the equation is rewritten as .  We will explore properties of taxicab circles further in ???? but I can't resist talking about now. In Euclidean geometry is defined as the ratio of the circumference to the diameter of a circle. So, let's try to use that definition for taxicab circles. shows a taxicab circle with centre and radius , say, as well as a straight line passing through .   A taxicab circle and a diameter    To determine pi we need to determine the ratio of the circumference to the diameter of the circle.     To determine the circumference of the circle we need to first determine the length of one of the sides of the circle, for example the interval . Now, we can use the taxicab distance formula to determine the taxicab distance between the points and but we have the problem that the path used to determine this distance doesn't coincide with the side . For the moment then, let's make the following decision. We will look at the idea of the length of an interval in more detail in ????    length of an interval   The length of the interval joining the points and is given by the taxicab distance between the endpoints of the interval,  and .    With this definition, we can now determine that the length of side  is . This is because the taxicab distance between and is , (which you can see by using the taxicab path , ). Since the taxicab circle consists of four sides the circumference of the circle is . Similarly the diameter of the circle will be the length of the interval which is, using , . Thus, so long as we are happy with , we have:    In taxicab geometry .      Taxicab Angles  As mentioned earlier, the taxicab geometry considered by Kraus is the same as Euclidean co-ordinate geometry except for the way that distance is calculated. However Thompson and Dray explore a variation of taxicab geometry which has another distinction from Euclidean co-ordinate geometry, that being the way in which angles are determined. As is often the way in developing taxicab geometry ideas, Thompson and Dray's approach is guided by ideas from Euclidean geometry.  In Euclidean geometry, the radian is an often used measure of an angle. Following the Wikipedia article on the Radian we can define one radian as follows:   radian   One radian , (denoted by ), is defined to be the measure of the angle subtended at the centre of a circle which intercepts an arc equal in length to the radius of the circle.    See the Wikipedia article for a dynamic diagram illustrating this definition. Of course the circle used in defining the radian is the Euclidean circle. So, to define the taxicab radian it seems reasonable to make a similar definition but using a taxicab circle instead.   tradian   One taxicab radian, which we will call a tradian , (denoted by ), is defined to be the taxicab measure of the angle subtended at the centre of a taxicab circle which intercepts an arc equal in (taxicab) length to the radius of the circle.    Note that in both Euclidean geometry and taxicab geometry, if the circle used to determine the measure of an angle is a circle of radius 1 ( a unit circle), then the measure of the angle is numerically the same as the length of the arc of the circle that the angle intercepts. Using this observation, Figure shows an angle whose measure is 1 tradian.   Arc of length 1 on the unit taxicab circle    If you click on the checkbox in Figure to view the Euclidean measure of the angle whose taxicab measure is 1 tradian then you will notice an interesting feature of the tradian. For an angle of a given taxicab measure, the Eucidean measure of that angle depends upon the orientation of the angle! Another way of saying this is to say that, for an angle of a given taxicab measure, the fraction of a full turn that you would make to go from facing one arm of the angle to the other arm of the angle depends on the orientation of the angle. For this reason Thompson and Dray introduce the notion of the taxicab measure of an angle in standard position. An angle is said to be in standard position if one of the arms of the angle is parallel to the -axis.  Despite the fact that the taxicab measure of an angle depends on the orientation of the angle we can still work with the idea of tradians. Firstly, to determine the taxicab measure of some angle in the plane you would first draw a taxicab circle of radius 1 with it's centre on the vertex of the angle. You would then extend, if necessary, the arms, and of the angle so that they intercepted the circle. Then the taxicab measure of the angle would be given by the taxicab length of the arc subtended by the angle. For an example see .   Arc of length 1 on the Unit taxicab circle    Full turn has taxicab measure of 8 tradians, half turn (in standard position?) has a taxicab measure of 4 tradians and a quarter turn (in standard position?) has a taxicab measure of 2 tradians.  The angle sum of a triangle will be 4 tradians.  The taxicab plane .   "
},
{
  ...
  "number": "",
  "title": "",
  "body": "Euclidean coordinate geometry "
},
{
  "id": "tax-introduction-2-8",
  "level": "2",
  "url": "tax-introduction.html#tax-introduction-2-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "tax-introduction-2-9",
  "level": "2",
  "url": "tax-introduction.html#tax-introduction-2-9",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "taxicab distance "
},
{
  "id": "figure-taxicab-path",
  "level": "2",
  "url": "tax-introduction.html#figure-taxicab-path",
  "type": "Figure",
  "number": "1.1",
  "title": "",
  "body": " Taxicab paths between two points   "
},
{
  "id": "tax-introduction-2-13",
  "level": "2",
  "url": "tax-introduction.html#tax-introduction-2-13",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "taxicab path taxicab set "
},
{
  "id": "tax-introduction-3-6",
  "level": "2",
  "url": "tax-introduction.html#tax-introduction-3-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "taxicab circle "
...
{
  "id": "figure-taxicab-circle",
  "level": "2",
  "url": "tax-introduction.html#figure-taxicab-circle",
  "type": "Figure",
  "number": "1.2",
  "title": "",
  "body": " Taxicab circle with centre C and radius r   "
},
{
  "id": "figure-taxicab-pi",
  "level": "2",
  "url": "tax-introduction.html#figure-taxicab-pi",
  "type": "Figure",
  "number": "1.3",
  "title": "",
  "body": " A taxicab circle and a diameter    To determine pi we need to determine the ratio of the circumference to the diameter of the circle.    "
},
{
  "id": "definition-interval-length",
  "level": "2",
  "url": "tax-introduction.html#definition-interval-length",
  "type": "Definition",
  "number": "1.4",
  "title": "",
  "body": " length of an interval   The length of the interval joining the points and is given by the taxicab distance between the endpoints of the interval,  and .   "
},
{
  "id": "tax-introduction-3-14",
  "level": "2",
  "url": "tax-introduction.html#tax-introduction-3-14",
  "type": "Claim",
  "number": "1.5",
  "title": "",
  "body": "  In taxicab geometry .   "
},
{
  "id": "definition-radian",
  "level": "2",
  "url": "tax-introduction.html#definition-radian",
  "type": "Definition",
  "number": "1.6",
  "title": "",
  "body": " radian   One radian , (denoted by ), is defined to be the measure of the angle subtended at the centre of a circle which intercepts an arc equal in length to the radius of the circle.   "
},
{
  "id": "definition-tradian",
  "level": "2",
  "url": "tax-introduction.html#definition-tradian",
  "type": "Definition",
  "number": "1.7",
  "title": "",
  "body": " tradian   One taxicab radian, which we will call a tradian , (denoted by ), is defined to be the taxicab measure of the angle subtended at the centre of a taxicab circle which intercepts an arc equal in (taxicab) length to the radius of the circle.   "
},
{
  "id": "diagram-one-tradian",
  "level": "2",
  "url": "tax-introduction.html#diagram-one-tradian",
  "type": "Figure",
  "number": "1.8",
  "title": "",
  "body": " Arc of length 1 on the unit taxicab circle   "
},
{
  "id": "tax-introduction-4-9",
  "level": "2",
  "url": "tax-introduction.html#tax-introduction-4-9",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "diagram-taxicab-angle-measure",
  "level": "2",
  "url": "tax-introduction.html#diagram-taxicab-angle-measure",
  "type": "Figure",
  "number": "1.9",
  "title": "",
  "body": " Arc of length 1 on the Unit taxicab circle   "
...
},
{
  ...
  "number": "",
  "title": "References",
  "body": " References  Eugene F. Krause, Taxicab Geometry , Addison-Wesley, Menlo Park, California, 1976.  Kevin Thompson, Tevian Dray, Taxicab Angles and Trigonometry , The Pi Mu Epsilon Journal, Vol 11, No 2, 2000.  "
},
{
  "id": "backmatter-5",
  "level": "1",
